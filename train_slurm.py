...
def run(config):
    ...
    # Import the model--this line allows us to dynamically select different files. # noqa
    ...
    utils.write_metadata(config['logs_root'], experiment_name, config,
                         state_dict)
    # Prepare data; the Discriminator's batch size is all that needs to be passed # noqa
    ...
    for epoch in range(state_dict['epoch'], config['num_epochs']):
        ...
        for i, (x, y) in enumerate(pbar):
            ...
            state_dict['itr'] += 1
            # Make sure G and D are in training mode, just in case they got set to eval  # noqa
            # For D, which typically doesn't have BN, this shouldn't matter much. # noqa
            G.train()
            D.train()
            if config['ema']:
                G_ema.train()
