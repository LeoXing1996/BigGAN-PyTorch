...
# -*- coding: utf-8 -*-
''' Utilities file
This file contains utility functions for bookkeeping, logging, and data loading.  # noqa
...
def prepare_parser():
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help=('Use the multi-epoch sampler for dataloader? '
             '(default: %(default)s)'))
    ...
    parser.add_argument(
        ...
        default='SN',
        help=
        'Parameterization style to use for G, spectral norm (SN) or SVD (SVD)'
    ...
    parser.add_argument(
        ...
        default='SN',
        help=
        'Parameterization style to use for D, spectral norm (SN) or SVD (SVD)'
        ' or None (default: %(default)s)')
    parser.add_argument('--G_ch',
                        type=int,
                        default=64,
                        help='Channel multiplier for G (default: %(default)s)')
    parser.add_argument('--D_ch',
                        type=int,
                        default=64,
    ...
    parser.add_argument('--num_G_accumulations',
                        type=int,
                        default=1,
    ...
    parser.add_argument('--num_D_accumulations',
                        type=int,
                        default=1,
    ...
    parser.add_argument(
        ...
        default=False,
        help=
        'Run D twice rather than concatenating inputs? (default: %(default)s)')
    parser.add_argument(
        '--num_epochs',
        type=int,
        default=100,
    ...
    parser.add_argument('--num_save_copies',
                        type=int,
                        default=2,
    ...
    parser.add_argument(
        ...
        default='',
        help=
        'Optionally override the automatic experiment naming with this arg. '
        '(default: %(default)s)')
    parser.add_argument(
        '--config_from_name',
        action='store_true',
    ...
    parser.add_argument('--ema',
                        action='store_true',
                        default=False,
                        help='Keep an ema of G'
                        's weights? (default: %(default)s)')
    ...
    parser.add_argument(
        ...
        default=False,
        help=
        'Use the EMA parameters of G for evaluation? (default: %(default)s)')
    parser.add_argument(
        '--ema_start',
        type=int,
        default=0,
    ...
    parser.add_argument('--num_G_SV_itrs',
                        type=int,
                        default=1,
                        help='Number of SV itrs in G (default: %(default)s)')
    parser.add_argument('--num_D_SV_itrs',
                        type=int,
                        default=1,
    ...
    parser.add_argument('--toggle_grads',
                        action='store_true',
                        default=True,
                        help='Toggle D and G'
    ...
    parser.add_argument('--which_train_fn',
                        type=str,
                        default='GAN',
    ...
    parser.add_argument(
        ...
        '(default: %(default)s)')
    parser.add_argument('--sv_log_interval',
                        type=int,
                        default=10,
...
def add_sample_parser(parser):
    parser.add_argument('--sample_npz',
                        ...
                        '(default: %(default)s)')
    parser.add_argument('--sample_num_npz',
                        type=int,
                        default=50000,
    ...
    parser.add_argument('--sample_interps',
                        action='store_true',
                        default=False,
    ...
    parser.add_argument(
        ...
        default=False,
        help=
        'Calculate Inception metrics with sample.py? (default: %(default)s)')
    return parser
  ...
  """
    ...
    def __len__(self):
        return len(self.data_source
                   ) * self.num_epochs - self.start_itr * self.batch_size
...
def get_data_loaders(dataset,
    ...
    train_set = which_dataset(root=data_root,
...
class ema(object):
    ...
    def update(self, itr=None):
        ...
        with torch.no_grad():
            for key in self.source_dict:
                self.target_dict[key].data.copy_(self.target_dict[key].data *
                                                 decay +
                                                 self.source_dict[key].data *
                                                 (1 - decay))
...
def load_weights(G,
    ...
    for item in state_dict:
        state_dict[item] = torch.load(
            '%s/%s.pth' %
            (root, join_strings('_', ['state_dict', name_suffix])))[item]
    if G_ema is not None:
        G_ema.load_state_dict(torch.load(
            '%s/%s.pth' % (root, join_strings('_', ['G_ema', name_suffix]))),
                              strict=strict)
        ...
        if os.path.exists('%s/%s.log' % (self.root, item)):
            if self.reinitialize:
                ...
                if 'sv' in item:
                    ...
                    print('{} exists, deleting...'.format('%s_%s.log' %
                                                          (self.root, item)))
...
def write_metadata(logs_root, experiment_name, config, state_dict):
    with open(('%s/%s/metalog.txt' % (logs_root, experiment_name)),
              'w') as writefile:
...
def progress(items, desc='', total=None, min_delay=0.1, displaytype='s1k'):
  ...
  """
    ...
    print("\r%s%d/%d (100.00%%) (took %d:%02d)" %
...
def sample_sheet(G,
    ...
    for i in range(num_classes // classes_per_sheet):
        ...
        for j in range(samples_per_class):
            if (z_ is not None) and hasattr(
                    z_, 'sample_') and classes_per_sheet <= z_.size(0):
                z_.sample_()
            else:
        ...
        out_ims = torch.stack(ims,
                              1).view(-1, ims[0].shape[1], ims[0].shape[2],
...
def interp_sheet(G,
    ...
    if fix_y:  # If fix y, only sample 1 z per row
        ...
        ys = interp(
            G.shared(sample_1hot(num_per_sheet,
                                 num_classes)).view(num_per_sheet, 1, -1),
            G.shared(sample_1hot(num_per_sheet,
                                 num_classes)).view(num_per_sheet, 1, -1),
    ...
    image_filename = '%s/%s/%d/interp%s%d.jpg' % (
        samples_root, experiment_name, folder_number, interp_style,
        sheet_number)
    torchvision.utils.save_image(out_ims,
                                 image_filename,
...
def print_grad_norms(net):
    ...
    print([
        '%3.3e,%3.3e, %s' % (gradsums[item_index][0], gradsums[item_index][1],
...
def accumulate_standing_stats(net, z, y, nclasses, num_accumulations=16):
    ...
    for i in range(num_accumulations):
        with torch.no_grad():
            ...
            x = net(z, net.shared(
                y))  # No need to parallelize here unless using syncbn
    # Set to eval mode
    net.eval()
