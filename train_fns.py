...
def GAN_training_function(G, D, GD, z_, y_, ema, state_dict, config):
    def train(x, y):
        ...
        for step_index in range(config['num_D_steps']):
            # If accumulating gradients,
            # loop multiple times before an optimizer step
            D.optim.zero_grad()
        ...
        if config['G_ortho'] > 0.0:
            ...
            # Don't ortho reg shared, it makes no sense.
            # Really we should blacklist any embeddings for this
            utils.ortho(G,
                        config['G_ortho'],
...
def save_and_sample(G, D, G_ema, z_, y_, fixed_z, fixed_y, state_dict, config,
    ...
    utils.sample_sheet(
        ...
        z_=z_)
    # Also save interp sheets  --> TODO: temp ignore
...
def test(G,
    ...
    print('Itr %d: PYTORCH UNOFFICIAL Inception Score is %3.3f +/- %3.3f, '
          'PYTORCH UNOFFICIAL FID is %5.4f' %
    ...
    test_log.log(itr=int(state_dict['itr']),
                 ...
                 IS_std=float(IS_std),
                 FID=float(FID))
    # save to writer if have
    if writer is not None:
        writer.add_scalars(
            'metric',
            dict(IS_mean=float(IS_mean),
                 IS_std=float(IS_std),
                 FID=float(FID)),
            int(state_dict['itr']))
